...
### Step 2: RGB 색상 톤 분석 (가장 중요! 반드시 이 순서로!)
각 날짜 셀을 분석할 때:

**1단계: RGB 값 추출**
- 배경색의 R, G, B 값을 정확히 추출하세요

**2단계: B와 G 비교 (절대적 규칙)**
- **B > G 이면 → 무조건 "근무"** (파란색 계열)
- **G > B 이면 → 무조건 "휴무"** (초록색 계열)
- **B = G 이면 → Step 3으로 (회색 계열 가능성)**
...
- RGB(200, 200, 200): B = G → 회색 계열 → Step 3
...
[
  {
    ...
    "type": "휴무",
    "reasoning": "Step 2: RGB 톤 분석 - 배경색이 #B0BF08(올리브 그린)와 유사, RGB(176, 191, 8), G값(191)이 가장 높아 초록색 계열로 판단. 기준 색상 #B0BF08와 일치. 최종: 휴무"
  },
  {
    "date": "2026-01-02",
    "color": "#0082EB 또는 유사한 파란색",
    "type": "근무",
    "reasoning": "Step 2: RGB 톤 분석 - 배경색이 #0082EB(진한 파란색)와 유사, RGB(0, 130, 235), B값(235)이 가장 높아 파란색 계열로 판단. 기준 색상 #0082EB와 일치. 최종: 근무"
  },
  {
    "date": "2026-01-03",
    "color": "#57BBE7 또는 유사한 하늘색",
    "type": "근무",
    "reasoning": "Step 2: RGB 톤 분석 - 배경색이 #57BBE7(하늘색)와 유사, RGB(87, 187, 231), B값(231)이 가장 높아 파란색 계열로 판단. 기준 색상 #57BBE7와 일치. 최종: 근무"
  }
]
...
3. **절대값 기준 확인했나요?**
   - B ≥ 150 → **"근무"**
   - G ≥ 150 and B ≤ 50 → **"휴무"**
4. **reasoning에 RGB 값과 판단 근거를 명시했나요?**
